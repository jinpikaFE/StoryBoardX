...
import { StoryboardConfig, LLMMessage, StreamChunk } from '../../common/interfaces/storyboard.interface';
import {
  STYLE_MAP,
  PACE_MAP,
  LANGUAGE_MAP,
...
export class LlmService {
  ...
  private buildSystemPrompt(config: StoryboardConfig): string {
    const battleModeFormat = config.promptMode === 'battle'
      ? `
【战斗模式专属字段】
每个方案必须包含以下额外字段：
...
${config.promptMode === 'battle'
  ...
  : '你需要根据用户提供的剧情概要和参数设置，生成2套不同风格的详细电影级分镜提示词方案。'}
${battleModeFormat}
...
- 镜头时长总和必须等于总时长`;
  ...
  private buildUserPrompt(config: StoryboardConfig): string {
    let dialogueInfo = '- 不包含台词';
    if (config.includeDialogue) {
      dialogueInfo = `- 包含台词/对白
- 台词语言：${LANGUAGE_MAP[config.dialogueLanguage] || config.dialogueLanguage}
- 旁白风格：${NARRATION_MAP[config.narrationStyle] || config.narrationStyle}`;
    }

    let promptModeInfo = '';
    if (config.promptMode === 'scene') {
      promptModeInfo = '场景模式：用户将看到"场景一：开场即高燃 · 兵器相接"这样的标题格式';
    } else if (config.promptMode === 'battle') {
      promptModeInfo = '场景战斗模式：用户将看到详细的战斗分镜格式';
    } else {
      promptModeInfo = '时间模式：用户将看到"0-4秒画面:"这样的时间标题格式';
    }

    let soundNote = '';
    if (config.musicStyle === 'silent-sfx') {
      soundNote = `
【重要音效要求】
...
- 视觉风格：${STYLE_MAP[config.visualStyle] || config.visualStyle}
- 叙事节奏：${PACE_MAP[config.narrativePace] || config.narrativePace}
- 音效/配乐倾向：${MUSIC_MAP[config.musicStyle] || config.musicStyle}
...
请生成${config.promptMode === 'battle' ? '1套完整的战斗分镜方案' : '2套风格差异明显的分镜方案'}，输出JSON数组格式。`;
  ...
  async *generateStoryboard(config: StoryboardConfig): AsyncGenerator<StreamChunk> {
